  ...
  return (
    <div className="w-64 h-full bg-sidebar flex flex-col border-r border-sidebar-border shadow-xl z-40">
      ...
      <div className="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto custom-scrollbar">
        <p className="px-4 text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest mb-2">Main Navigation</p>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <item.icon className={cn("h-5 w-5 transition-colors duration-300", isActive ? "text-white" : "text-muted-foreground/50 group-hover:text-sidebar-foreground")} />
                {item.label}
              </div>
      ...
      <div className="p-6 border-t border-sidebar-border/50">
        <div className="bg-sidebar-accent/50 rounded-2xl p-4 border border-sidebar-border">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest">System Pulse</p>
